...
QMap<Option, QVariant> ClockSettings::GetSettings() const
{
  ...
  all_settings[OPT_HANDLE_INPUT_ON_CTRL]  = GetValue(OPT_HANDLE_INPUT_ON_CTRL);
  return all_settings;
}
...
QString ClockSettings::GetKey(const int id) const
{
  switch (static_cast<Option>(id)) {
    ...
    case OPT_HANDLE_INPUT_ON_CTRL:  return "window/handle_input_on_ctrl";
  }
  Q_ASSERT(false);
  return QString();
}
...
QVariant ClockSettings::GetDefaultValue(const int id) const
{
  ...
  switch (static_cast<Option>(id)) {
    ...
    case OPT_HANDLE_INPUT_ON_CTRL:  return false;
  }
  return QVariant();
}
